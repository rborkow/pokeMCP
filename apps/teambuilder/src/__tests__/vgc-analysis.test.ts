...
describe("VGC Analysis", () => {
  ...
  describe("analyzeVGCTeam", () => {
    ...
    it("warns when multiple Pokemon missing Protect", () => {
      const team = [
        createPokemon("Garchomp", ["Earthquake", "Dragon Claw", "Swords Dance", "Stone Edge"]),
        createPokemon("Landorus", ["Earth Power", "Sludge Bomb", "Psychic", "Focus Blast"]),
        createPokemon("Kingambit", ["Iron Head", "Sucker Punch", "Swords Dance", "Kowtow Cleave"]),
        createPokemon("Gholdengo", ["Make It Rain", "Shadow Ball", "Nasty Plot", "Thunderbolt"]),
      ];
      const warnings = analyzeVGCTeam(team);
      expect(warnings.some((w) => w.message.includes("Protect") || w.message.includes("protect"))).toBe(true);
    });

    it("does not warn about Protect for Assault Vest users", () => {
      const team = [
        createPokemon("Rillaboom", ["Grassy Glide", "Wood Hammer", "Fake Out", "U-turn"], "Assault Vest"),
        createPokemon("Incineroar", ["Flare Blitz", "Knock Off", "Fake Out", "U-turn"], "Assault Vest"),
        createPokemon("Garchomp", ["Earthquake", "Dragon Claw", "Protect", "Stone Edge"]),
        createPokemon("Kingambit", ["Iron Head", "Sucker Punch", "Protect", "Kowtow Cleave"]),
      ];
      const warnings = analyzeVGCTeam(team);
    ...
    it("does not warn about Protect for Choice item users", () => {
      const team = [
        createPokemon("Dragapult", ["Dragon Darts", "Phantom Force", "U-turn", "Draco Meteor"], "Choice Specs"),
        createPokemon("Garchomp", ["Earthquake", "Dragon Claw", "Protect", "Stone Edge"]),
        createPokemon("Kingambit", ["Iron Head", "Sucker Punch", "Protect", "Kowtow Cleave"]),
        createPokemon("Gholdengo", ["Make It Rain", "Shadow Ball", "Protect", "Thunderbolt"]),
      ];
      const warnings = analyzeVGCTeam(team);
    ...
    it("warns about missing speed control", () => {
      ...
      const warnings = analyzeVGCTeam(team);
      expect(warnings.some((w) => w.message.toLowerCase().includes("speed control"))).toBe(true);
    });
    ...
    it("does not warn about speed control when team has Tailwind", () => {
      ...
      const warnings = analyzeVGCTeam(team);
      expect(warnings.every((w) => !w.message.toLowerCase().includes("speed control"))).toBe(true);
    });
