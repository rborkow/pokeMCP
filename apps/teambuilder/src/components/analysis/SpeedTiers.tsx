    ...
    return (
        <div className={`flex items-center gap-3 p-2 rounded ${tierBg}`}>
            <div className="flex items-center gap-2 w-36">
                ...
                <span className="text-sm font-medium truncate">{toDisplayName(info.pokemon)}</span>
            </div>
...
function SpeedControlBadges({ control }: { control: ReturnType<typeof hasTeamSpeedControl> }) {
    const badges = [];

    if (control.hasTailwind) {
        badges.push(
            <span key="tw" className="px-2 py-1 text-xs bg-blue-500/20 text-blue-400 rounded">
                Tailwind
            </span>,
        );
    }
    if (control.hasTrickRoom) {
        badges.push(
            <span key="tr" className="px-2 py-1 text-xs bg-pink-500/20 text-pink-400 rounded">
                Trick Room
            </span>,
        );
    }
    if (control.hasIcyWind) {
        badges.push(
            <span key="iw" className="px-2 py-1 text-xs bg-cyan-500/20 text-cyan-400 rounded">
                Icy Wind
            </span>,
        );
    }
    if (control.hasElectroweb) {
        badges.push(
            <span key="ew" className="px-2 py-1 text-xs bg-yellow-500/20 text-yellow-400 rounded">
                Electroweb
            </span>,
        );
    }
    ...
    if (badges.length === 0) {
        return <span className="text-xs text-amber-500">No speed control detected</span>;
    }
                    ...
                    {mode === "vgc" && (
                        <div className="text-xs text-muted-foreground mt-2">
                            ...
                            <ul className="list-disc list-inside space-y-0.5">
                                ...
                                <li>Trick Room reverses speed order (slower moves first)</li>
