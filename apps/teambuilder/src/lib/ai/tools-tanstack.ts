/**
 * TanStack AI tool definitions for structured team modifications.
 *
 * Uses toolDefinition() from @tanstack/ai with needsApproval so tool calls
 * are surfaced to the client as ActionCards for user confirmation.
 */
import { toolDefinition } from "@tanstack/ai";

export const modifyTeamTool = toolDefinition({
    name: "modify_team" as const,
    ...
    inputSchema: {
        ...
        properties: {
            ...
            ivs: {
                ...
                description:
                    "IV spread (usually 31s, but 0 Atk for special attackers, 0 Spe for Trick Room)",
            },
            move_slot: {
                type: "number",
...
/**
 * Re-export ModifyTeamInput type — unchanged from the original tools.ts.
 * Kept in sync manually since TanStack AI uses JSON Schema (not Zod) for
 * tool input schemas, so we cannot infer the TS type automatically.
 */
export type { ModifyTeamInput } from "./tools";
