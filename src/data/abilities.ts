...
export const Abilities: import("../sim/dex-abilities").AbilityDataTable = {
    ...
    aromaveil: {
        onAllyTryAddVolatile(status, target, source, effect) {
            if (
                ["attract", "disable", "encore", "healblock", "taunt", "torment"].includes(
                    status.id,
                )
            ) {
    ...
    berserk: {
        ...
        onAfterMoveSecondary(target, source, move) {
            ...
            if (!lastAttackedBy) return;
            const damage =
                move.multihit && !move.smartTarget ? move.totalDamage : lastAttackedBy.damage;
            if (target.hp <= target.maxhp / 2 && target.hp + damage > target.maxhp / 2) {
        ...
        onBeforeSwitchIn(pokemon) {
            ...
            for (let i = pokemon.side.pokemon.length - 1; i > pokemon.position; i--) {
                ...
                if (!possibleTarget.fainted) {
                    ...
                    if (
                        !pokemon.terastallized ||
                        !["Ogerpon", "Terapagos"].includes(possibleTarget.species.baseSpecies)
                    ) {
    ...
    noguard: {
        ...
        onAnyAccuracy(accuracy, target, source, move) {
            if (
                move &&
                (source === this.effectState.target || target === this.effectState.target)
            ) {
                return true;
            }
            return accuracy;
    ...
    powerconstruct: {
        ...
        onResidual(pokemon) {
            ...
            pokemon.canMegaEvo =
                pokemon.canMegaEvo === false ? false : this.actions.canMegaEvo(pokemon);
