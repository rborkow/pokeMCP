...
export const Moves: import("../sim/dex-moves").MoveDataTable = {
    ...
    crushgrip: {
        ...
        basePowerCallback(pokemon, target) {
            ...
            const bp =
                Math.floor(
                    Math.floor((120 * (100 * Math.floor((hp * 4096) / maxHP)) + 2048 - 1) / 4096) /
                        100,
                ) || 1;
    ...
    dive: {
        ...
        onTryMove(attacker, defender, move) {
            ...
            if (
                ...
                const forme =
                    attacker.hp <= attacker.maxhp / 2 ? "cramorantgorging" : "cramorantgulping";
    ...
    doubleshock: {
        ...
        self: {
            onHit(pokemon) {
                pokemon.setType(
                    pokemon.getTypes(true).map((type) => (type === "Electric" ? "???" : type)),
                );
                this.add(
                    "-start",
                    pokemon,
    ...
    hardpress: {
        ...
        basePowerCallback(pokemon, target) {
            ...
            const bp =
                Math.floor(
                    Math.floor((100 * (100 * Math.floor((hp * 4096) / maxHP)) + 2048 - 1) / 4096) /
                        100,
                ) || 1;
            this.debug(`BP for ${hp}/${maxHP} HP: ${bp}`);
            return bp;
    ...
    shellsidearm: {
        ...
        onModifyMove(move, pokemon, target) {
            ...
            const physical = Math.floor(
                Math.floor(Math.floor(Math.floor((2 * pokemon.level) / 5 + 2) * 90 * atk) / def) /
                    50,
            );
            const special = Math.floor(
                Math.floor(Math.floor(Math.floor((2 * pokemon.level) / 5 + 2) * 90 * spa) / spd) /
                    50,
            );
