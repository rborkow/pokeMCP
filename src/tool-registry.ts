...
export const TOOL_REGISTRY: ToolDefinition[] = [
	// --- Sync tools (bundled Pokemon Showdown data, no env needed) ---
	{
		...
		schema: {
			pokemon: z
				.string()
				.describe(
					"The name of the Pokémon to look up (e.g., 'Pikachu', 'Charizard')",
				),
			generation: z
				.string()
				.optional()
	...
	},
	{
		...
		execute: async (args) =>
			validateMoveset(
				args as { pokemon: string; moves: string[]; generation?: string },
			),
	},
	{
		name: "validate_team",
	...
	},
	{
		...
		schema: {
			...
			format: z
				.string()
				.optional()
				.describe("Format for suggestions (e.g., 'OU')"),
		},
		execute: async (args) =>
	...
	},
	{
		...
		schema: {
			format: z
				.string()
				.optional()
				.describe("Format to check (e.g., 'gen9ou', 'gen9ubers')"),
	...
	},
	{
		...
		schema: {
			...
			format: z
				.string()
				.optional()
				.describe("Format to check (e.g., 'gen9ou')"),
			limit: z.number().optional().describe("Number of teammates to show"),
		},
		execute: async (args, env) =>
			getTeammates(
				args as { pokemon: string; format?: string; limit?: number },
				env,
			),
	},
	{
		...
		schema: {
			...
			limit: z
				.number()
				.optional()
				.describe("Number of checks/counters to show"),
		},
		execute: async (args, env) =>
			getChecksCounters(
				args as { pokemon: string; format?: string; limit?: number },
	...
	// --- RAG tools (Cloudflare Vectorize + KV, need env) ---
	{
		name: "query_strategy",
		description:
			"Natural language search over Smogon strategic content for competitive advice",
		schema: {
			query: z
				.string()
				.describe(
			...
			format: z
				...
				.optional()
				.describe(
					"Optional: Filter by format (e.g., 'gen9ou', 'gen9vgc2024regf')",
				),
			limit: z
				.number()
				.optional()
