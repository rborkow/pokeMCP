...
function parseLearnsetMethod(methodStr: string) {
    const gen = Number.parseInt(methodStr[0], 10);
...
export function validateMoveset(args: {
    ...
    const targetGen = args.generation ? Number.parseInt(args.generation, 10) : 9;
...
export function validateTeam(args: { team: TeamPokemon[]; format?: string }): string {
    ...
    for (const member of args.team) {
        ...
        const isGen9 =
            normalizedFormat.startsWith("gen9") ||
            normalizedFormat.includes("sv") ||
            normalizedFormat.includes("vgc202");

        if (member.teraType) {
            if (!isGen9) {
                errors.push(`${species.name}: Tera Type is only available in Gen 9 formats`);
            } else {
                const normalizedTera =
                    member.teraType.charAt(0).toUpperCase() +
                    member.teraType.slice(1).toLowerCase();
                ...
                if (species.requiredTeraType) {
                    const requiredTera =
                        species.requiredTeraType.charAt(0).toUpperCase() +
                        species.requiredTeraType.slice(1).toLowerCase();
    ...
    for (const pokemonName of team) {
        ...
        for (const attackingType of allTypes) {
            ...
            for (const defenderType of species.types) {
                ...
                if (effectiveness === 3)
                    totalEffectiveness = 0; // immune
                else if (effectiveness === 2)
                    totalEffectiveness *= 0.5; // resists
 ...
 */
export function suggestTeamCoverage(args: { current_team: string[]; format?: string }): string {
    const format = args.format || "OU";
    const results: string[] = [];
    ...
    if (weaknesses.size > 0) {
        ...
        const sortedWeaknesses = Array.from(weaknesses.entries()).sort((a, b) => b[1] - a[1]);
    ...
    if (resistances.size > 0) {
        ...
        const sortedResistances = Array.from(resistances.entries()).sort((a, b) => b[1] - a[1]);
